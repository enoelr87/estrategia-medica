...
export class DeviceDetectorService {
	...
	public getDeviceInfo(): DeviceInfo {
		// Parche para Brave
		// @ts-ignore
		const browser  = navigator?.brave ? 'Brave' : this.browser;
	...
	public isMobile(userAgent = this.userAgent): boolean {
		...
		// @ts-ignore
			this.reTree.test(userAgent, Constants.MOBILES_RE[mobile])
		);
		return !!match;
	}
	...
	public isTablet(userAgent = this.userAgent): boolean {
		...
		// @ts-ignore
			!!this.reTree.test(userAgent, Constants.TABLETS_RE[mobile])
		);
		return !!match;
	}
